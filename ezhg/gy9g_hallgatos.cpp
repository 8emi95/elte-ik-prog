...
/*
Rekord elemtípusú mátrix beolvasása – ami ráadásul kesztyûmátrix, azaz különbözõ hosszúak a sorai; beolvasás soronként, sorok feldolgozása stringstreammel; egymásba ágyazott tételek; lineáris keresések, maximumkiválasztás; „idegenkulcs-kapcsolat” vektor felé

Adottak a programozás csoportok félév végi jegyei egy mátrixban. Minden sora egy csoport, és azon belül minden elem egy neptun-jegy páros (0, ha nincs jegy). Minden csoport legalább egy tagú.
1. Volt-e olyan csoport a legalább 2 tagúak közül, ahol mindenki megbukott (tehát 0 vagy 1 a jegye) (pesszimista lineáris keresés, benne optimista lineáris keresés)
2. Adott egy listában a csoportok sorszáma szerint a gyakorlatvetõk neve. Kinek a csoportjában volt a legjobb hallgató? (maximumkiválasztásban maximumkiválasztás – a végén le kell kérni a tanár nevét, tehát ez egy összetett függvény)
*/
...
bool csoportokatBeolvas(vector<vector<Hallgato> > &csoportok)
{
    ...
    while(getline(ifs,csoport)) //Egy sor - egy csoport
    {
        ...
        csoportok.resize(csoportok.size() + 1); //Ezzel beszurom a (kezdetben ures) csoportot
        ...
        while(getline(ss,hallgato,' ')) //Hallgatok adatai egymastol szokozzel vannak elvalasztva...
        {
            ...
            if(h.jegy < 0 || h.jegy > 5)    //Ha nem [0..5]-beli
            {
                ifs.close();
                return false;
            }
